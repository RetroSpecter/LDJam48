...
public class IngredientQueueUI : MonoBehaviour {
    ...
    public void setupUI(List<ingredientDrop> inventory, IngredientsList ingredients) {
        ...
        for (int i = 0; i < images.Length; i++) {
           ...
           // FoodUI newFood = Instantiate(foodObj, counter.transform.position, Quaternion.identity).GetComponent<FoodUI>();
            ...
            //foodUI.Add(newFood);
            images[i].sprite = IngredientsList.getIngredient(inventory[i].name).ingredintImage;
    ...
    public void updateUI(List<ingredientDrop> inventory, IngredientsList ingredients) {
        counter.text = "" + inventory.Count;
